...
void webSocket::startThread(int port)
{
	webthread = std::thread(&webSocket::startServer, this);
}
...
//		std::unique_lock<std::mutex> mlock(_mutexOutput);
//		_condOutput.wait_for(mlock, std::chrono::milliseconds(5000));
		...
		for (int i = 0; i < wsClients.size(); ++i)
		{
			if (wsClients[i] != NULL && wsClients[i]->ReadyState == WS_READY_STATE_OPEN)
			{
				...
				buffer << "{\"Distance\":\"" << ddd.get("Distance") << "\",";
				buffer << "\"FrontLeft\":\"" << ddd.get("FrontLeft") <<"\",";
				buffer << "\"RearLeft\":\"" << ddd.get("RearLeft") << "\",";
				buffer << "\"RearRight\":\"" << ddd.get("RearRight") << "\",";
				buffer << "\"FrontRight\":\"" << ddd.get("FrontRight") << "\"";
				buffer << "}";

				transmitJSON(i, buffer.str());
			}
		}
		...
		if (cameras.empty() == false)
		{
			...
			buffer << "{\"cameras\": [";
				...
				//buffer << "{\"id\":3, \"markers\" : [{\"points2d\":[{\"x\":284, \"y\" : 353},
			int index = 0;
			for (auto& c : cameras)
			{
				buffer << "{ \"id\" : \"" << c.first << "\", \"status\" : \"" << c.second << "\"}";
...
int webSocket::transmitJSON(int clientID, std::string& buffer)
{
	std::vector < unsigned char > uchar_encoded;
	std::string base64encoded;

	if (!wsSend(clientID, buffer))
...
void webSocket::notify(WheelMeasure& distances)
{
	ddd = distances;
}

/*
void webSocket::notify(Message& msg)
{
	bool to_unlock = false;
	if (msg._type == Message::CAMERA_ACQUISITION)
	{
		cameras[msg.id] = msg.body;
		//std::cout << msg._type << ": " <<msg.id <<" - " << msg.body << std::endl;
		to_unlock = true;
	}
	if (msg._type == Message::TARGET_DEFINITION)
	{
		targets[msg.id] = msg.body;
		//std::cout << msg._type << ": " << msg.body << std::endl;
		to_unlock = true;
	}

	if (to_unlock && _mutexOutput.try_lock())
	{
		_mutexOutput.unlock();
		_condOutput.notify_one();
	}
